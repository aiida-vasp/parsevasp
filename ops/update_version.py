...
class VersionUpdater:
    ...
    * parsevasp/__init__.py
    ...
    The current version number is either parsed from the output of ``git describe --tags --match v*.*.*``, or
    if the command fails for any reason, from setup.json. The current version number is decided on init,
    syncronization can be executed by calling ``.sync()``.
    ...
    def __init__(self):
        ...
        self.top_level_init = py_path.local(subpath('parsevasp', '__init__.py'))
    ...
    def write_to_init(self):
        ...
        self.top_level_init.write(
            re.sub(
                self.init_version_pat, r'\1\g<2>{}\4'.format(str(self.version)), init_content, re.DOTALL | re.MULTILINE
            )
        )
