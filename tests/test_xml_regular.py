...
def test_xml_energies(xml_parser):
    ...
    energy_data = xml_data.get_energies('last')
    assert np.allclose(test_array_energies, energies_data['energy_extrapolated'])
    assert np.allclose(test_array_steps, energies_data['electronic_steps'])
    energies_data = xml_data.get_energies('all')
    assert np.allclose(test_array_energies, energies_data['energy_extrapolated'])
    assert np.allclose(test_array_steps, energies_data['electronic_steps'])
    ...
    assert np.allclose(test_array_energies, energies_data['energy_extrapolated'])
    assert np.allclose(test_array_steps, energies_data['electronic_steps'])
    energies_data = xml_data.get_energies('last')
...
def test_xml_energies_electronic(xml_parser):
    ...
    assert np.allclose(test_array_energies, energies_data['energy_extrapolated'])
    assert np.allclose(test_array_steps, energies_data['electronic_steps'])
    energies_data = xml_data.get_energies('last', nosc=False)
    assert np.allclose(test_array_energies, energies_data['energy_extrapolated'])
    assert np.allclose(test_array_steps, energies_data['electronic_steps'])
    energies_data = xml_data.get_energies('all', nosc=False)
    assert np.allclose(test_array_energies, energies_data['energy_extrapolated'])
    ...
    # Ionic steps
    xml_data = xml_parser(filename='basicrelax.xml')
    energies_data = xml_data.get_energies('initial', nosc=False)
...
def test_xml_stress(xml_parser):
    ...
    stress = xml_data.get_stress('last')
    np.testing.assert_allclose(stress, test)
...
def test_xml_unitcell(xml_parser):
    ...
    unitcell = xml_data.get_unitcell('last')
    np.testing.assert_allclose(unitcell, test)
...
def test_xml_positions(xml_parser):
    ...
    test = np.array([[0., 0., 0.], [0., 0.50000092, 0.50000092],
    ...
    positions = xml_data.get_positions('last')
    np.testing.assert_allclose(positions, test)
...
def test_xml_kpoints(xml_parser):
    ...
    test = np.array([[0., 0., 0.], [0.16666667, 0., 0.], [0.33333333, 0., 0.],
                     [0.5, 0., 0.], [0.16666667, 0.16666667, 0.],
                     [0.33333333, 0.16666667, 0.], [0.5, 0.16666667, 0.],
                     [0.33333333, 0.33333333, 0.], [0.5, 0.33333333, 0.],
                     [0.5, 0.5, 0.], [0.16666667, 0.16666667, 0.16666667],
                     [0.33333333, 0.16666667, 0.16666667],
                     [0.5, 0.16666667, 0.16666667],
                     [0.33333333, 0.33333333, 0.16666667],
                     [0.5, 0.33333333, 0.16666667], [0.5, 0.5, 0.16666667],
...
def test_xml_kpointsw(xml_parser):
    ...
    test = np.array([
        0.00462963, 0.02777778, 0.02777778, 0.01388889, 0.05555556, 0.11111111,
        0.05555556, 0.05555556, 0.05555556, 0.01388889, 0.03703704, 0.11111111,
        0.05555556, 0.11111111, 0.11111111, 0.02777778, 0.03703704, 0.05555556,
...
def test_xml_dos(xml_parser):
    ...
    test = np.array([
        -8.1993, -8.1307, -8.0621, -7.9935, -7.9249, -7.8563, -7.7877, -7.7191,
        -7.6505, -7.5819, -7.5133, -7.4447, -7.376, -7.3074, -7.2388, -7.1702,
        -7.1016, -7.033, -6.9644, -6.8958, -6.8272, -6.7586, -6.69, -6.6214,
        9.2958, 9.3644, 9.433, 9.5016, 9.5702, 9.6388, 9.7075, 9.7761, 9.8447,
        9.9133, 9.9819, 10.0505, 10.1191, 10.1877, 10.2563, 10.3249, 10.3935,
        10.4621, 10.5308, 10.5994, 10.668, 10.7366, 10.8052, 10.8738, 10.9424,
        11.011, 11.0796, 11.1482, 11.2168, 11.2855, 11.3541, 11.4227, 11.4913,
        11.5599, 11.6285, 11.6971, 11.7657, 11.8343, 11.9029, 11.9715, 12.0401,
        12.1088, 12.1774, 12.246, 12.3146, 12.3832
    ])
    ...
    test = np.array([
        ...
        1.00000000e-03, 3.70000000e-03, 1.19000000e-02, 3.26000000e-02,
        7.62000000e-02, 1.53300000e-01, 2.66500000e-01, 4.03000000e-01,
        5.35100000e-01, 6.36900000e-01, 7.08700000e-01, 7.85700000e-01,
        9.11300000e-01, 1.08720000e+00, 1.25030000e+00, 1.31500000e+00,
        1.25610000e+00, 1.15020000e+00, 1.12150000e+00, 1.23750000e+00,
        1.45800000e+00, 1.68130000e+00, 1.82360000e+00, 1.86180000e+00,
        1.83370000e+00, 1.81920000e+00, 1.88660000e+00, 2.01500000e+00,
        2.08500000e+00, 2.00140000e+00, 1.82570000e+00, 1.74490000e+00,
        1.90040000e+00, 2.27240000e+00, 2.71870000e+00, 3.09640000e+00,
        3.35090000e+00, 3.50170000e+00, 3.55490000e+00, 3.47540000e+00,
        3.26920000e+00, 3.03930000e+00, 2.90000000e+00, 2.84830000e+00,
        2.76630000e+00, 2.58130000e+00, 2.40160000e+00, 2.43990000e+00,
        2.79200000e+00, 3.32570000e+00, 3.77330000e+00, 3.88710000e+00,
        3.55760000e+00, 2.88190000e+00, 2.11760000e+00, 1.50290000e+00,
        1.11840000e+00, 9.21700000e-01, 8.49300000e-01, 8.46700000e-01,
        8.61700000e-01, 8.76100000e-01, 9.33100000e-01, 1.09690000e+00,
        1.37400000e+00, 1.67100000e+00, 1.83580000e+00, 1.76320000e+00,
        1.47700000e+00, 1.11540000e+00, 8.34900000e-01, 7.11800000e-01,
        7.10700000e-01, 7.45800000e-01, 7.96500000e-01, 9.71500000e-01,
        1.44190000e+00, 2.29450000e+00, 3.42890000e+00, 4.58890000e+00,
        5.49890000e+00, 5.97930000e+00, 5.94670000e+00, 5.37640000e+00,
        4.36070000e+00, 3.18920000e+00, 2.25920000e+00, 1.84460000e+00,
        1.95070000e+00, 2.34570000e+00, 2.68260000e+00, 2.67100000e+00,
        2.25980000e+00, 1.67430000e+00, 1.22390000e+00, 1.06650000e+00,
        1.15830000e+00, 1.37540000e+00, 1.63440000e+00, 1.91430000e+00,
        2.19620000e+00, 2.39300000e+00, 2.36200000e+00, 2.03010000e+00,
        1.49410000e+00, 9.60800000e-01, 5.99100000e-01, 4.64200000e-01,
        5.15700000e-01, 6.66900000e-01, 8.43600000e-01, 1.03810000e+00,
        2.96000000e-02, 8.40000000e-03, 1.90000000e-03, 4.00000000e-04,
...
def test_xml_dos_specific(xml_parser):
    ...
    assert dos['total']['partial'] == None
    test = np.array([
        -7.9387, -7.8022, -7.6656, -7.529, -7.3924, -7.2558, -7.1192, -6.9826,
        -6.8461, -6.7095
    ])
    np.testing.assert_allclose(dos['total']['energy'], test)
...
def test_xml_eigenvalues_specific(xml_parser):
    ...
    test = np.array([
        -6.076, -6.0578, -6.0031, -5.9122, -5.7858, -5.6246, -5.4302, -5.2048,
        -4.9517, -4.6768
    ])
    np.testing.assert_allclose(eigenvalues['total'][0], test)
    test = np.array([
        9.5694, 9.6943, 10.0793, 10.646, 11.2894, 11.9561, 12.6122, 13.2292,
        13.6978, 13.9563
    ])
    np.testing.assert_allclose(eigenvalues['total'][7], test)
    test = np.array([[0., 0., 0.], [0.03703704, 0., 0.], [0.07407407, 0., -0.],
                     [0.11111111, 0., -0.], [0.14814815, 0., -0.],
                     [0.18518519, -0., 0.], [0.22222222, -0., 0.],
                     [0.25925926, -0., 0.], [0.2962963, -0., -0.],
                     [0.33333333, -0., -0.]])
    np.testing.assert_allclose(kpoints, test)
    test = np.array([
        5.0810e-05, 4.0644e-04, 4.0644e-04, 4.0644e-04, 4.0644e-04, 4.0644e-04,
...
def test_xml_eigenvelocities(xml_parser):
    ...
    np.testing.assert_allclose(kpoints, test)
    test = np.array([
        5.081e-05, 5.081e-05, 5.081e-05, 5.081e-05, 5.081e-05, 5.081e-05,
...
def test_xml_dos_spin(xml_parser):
    ...
    test = np.array([
        1.7104, 1.6289, 1.5543, 1.4882, 1.4315, 1.3845, 1.3467, 1.3172, 1.2945,
        1.2772
    ])
    np.testing.assert_allclose(dos['down']['total'][460:470], test)
    test = np.array([
        1.708, 1.6267, 1.5523, 1.4864, 1.43, 1.3833, 1.3457, 1.3164, 1.294,
        1.2767
    ])
...
def test_xml_dos_partial(xml_parser):
    ...
    test = np.array([
        0.1287, 0.1216, 0.1152, 0.1095, 0.1046, 0.1005, 0.0973, 0.0947, 0.0927,
        0.0912
    ])
    np.testing.assert_allclose(dos['total']['partial'][0, 460:470, 0], test)
    test = np.array([
        0.1429, 0.1345, 0.1268, 0.1201, 0.1144, 0.1097, 0.1059, 0.1031, 0.101,
        0.0995
    ])
...
def test_xml_dielectrics(xml_parser):
    ...
    test = np.array(
        [1.20757000e+01, 1.14969000e+01, 1.14969000e+01, 0.00000000e+00])
...
def test_xml_structure_magmom(xml_parser):
    ...
    unitcell_last = xml_data.get_unitcell('last')
    ...
    positions_last = xml_data.get_positions('last')
...
def test_xml_ionic(xml_parser):
    ...
    forces = xml_data.get_forces('all')
    stress = xml_data.get_stress('all')
    energies_sc = xml_data.get_energies('all', nosc=False)
...
def test_xml_version(xml_parser):
    """Test if the version can be parsed"""
    xml_data = xml_parser(filename='basic.xml')
    version = xml_data.get_version()
